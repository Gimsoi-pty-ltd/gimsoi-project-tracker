...
test.describe("Users API Tests", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    expect(
      adminSignupRes.status(),
      `Admin signup failed: ${await adminSignupRes.text()}`,
    ).toBe(201);
    ...
    expect(
      userSignupRes.status(),
      `User signup failed: ${await userSignupRes.text()}`,
    ).toBe(201);
  ...
  test.describe("PATCH /api/users/me/password (Self)", () => {
    test("User can change password with correct current password", async ({
      request,
    }) => {
