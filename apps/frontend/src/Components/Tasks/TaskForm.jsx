...
export default function TaskForm({ formData, setFormData, userOptions = [], parentTaskOptions = [] }) {
  ...
  const safeOwnerIds = Array.isArray(formData?.ownerIds) ? formData.ownerIds : [];
  const safeTeamIds = Array.isArray(formData?.teamIds) ? formData.teamIds : [];
    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          >
            ...
            {parentTaskOptions.map((taskItem) => (
              <option key={taskItem.id} value={taskItem.id}>{taskItem.title}</option>
            ))}
          </select>
        </div>
      ...
      <div>
        ...
        <div className="border border-gray-200 rounded-lg p-3 space-y-2 max-h-32 overflow-y-auto">
          {userOptions.length === 0 && <p className="text-sm text-gray-500">No users available yet.</p>}
          {userOptions.map((user) => (
