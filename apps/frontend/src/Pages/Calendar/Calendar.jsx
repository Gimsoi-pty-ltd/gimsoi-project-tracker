...
const Calendar = () => {
  ...
  // Fetch events from backend — falls back gracefully if endpoint doesn't exist yet
  useEffect(() => {
    const fetchEvents = async () => {
      setIsLoading(true);
      try {
        const response = await resourceAPI.get(
          `/events?month=${currentMonth + 1}&year=${currentYear}`
        );
        setCalendarEvents(response.data.events || response.data || []);
      } catch {
  ...
  const changeYear        = (offset) => setCurrentDate(new Date(currentYear + offset, currentMonth, 1));

  const handleAddEvent = async () => {
    if (!selectedDate || !newEvent.title) return;
    try {
      const response = await resourceAPI.post("/events", {
        ...newEvent,
        date: typeof selectedDate === "string" ? selectedDate : selectedDate.toISOString().split("T")[0],
      });
      const created = response.data.event || response.data;
  ...
  const calendarDays = [];
  for (let i = firstDay - 1; i >= 0; i--) calendarDays.push({ day: daysInPrevMonth - i, currentMonth: false });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Tasks & Meetings Overview</p>
          </div>

          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <button onClick={goToToday} className="px-4 py-2 text-sm border border-gray-300 rounded-md bg-white hover:bg-gray-50 transition font-medium">
              Today
            </button>
            <div className="flex gap-2">
              <button onClick={goToPreviousMonth} className="px-3 py-2 rounded-md bg-[#002D62] hover:bg-[#001f44] text-white transition-all">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={goToNextMonth} className="px-3 py-2 rounded-md bg-[#002D62] hover:bg-[#001f44] text-white transition-all">
                <ChevronRight className="w-5 h-5" />
              </button>
            </div>
            <div className="flex gap-2">
              <button onClick={() => changeYear(-1)} className="px-3 py-2 text-sm border border-gray-300 rounded-md bg-white hover:bg-gray-50">← Year</button>
              <button onClick={() => changeYear(1)} className="px-3 py-2 text-sm border border-gray-300 rounded-md bg-white hover:bg-gray-50">Year →</button>
            </div>
            <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="grid grid-cols-7 bg-gray-50 border-b border-gray-200">
            {days.map((day) => (
              <div key={day} className="py-3 text-center text-xs font-semibold text-gray-500 tracking-wide uppercase">
                {day}
              </div>
            ))}
          </div>
          ...
          {isLoading ? (
            <div className="h-64 flex items-center justify-center text-gray-400 text-sm">Loading events...</div>
          ) : (
            <div className="grid grid-cols-7">
              {calendarDays.map((dayObj, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`min-h-32 border-b border-r border-gray-100 p-3 relative hover:bg-blue-50 transition cursor-pointer ${!isCurrentMonth ? "bg-gray-50" : ""} ${isToday ? "bg-blue-100" : ""}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {calendarEvents.filter((e) => e.type === "task").slice(0, 5).map((event) => (
              ...
              {calendarEvents.filter((e) => e.type === "task").length === 0 && (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              {calendarEvents.filter((e) => e.type === "meeting").slice(0, 5).map((event) => (
                <div key={event.id} className="flex gap-3 pb-3 border-b last:border-b-0">
              ...
              {calendarEvents.filter((e) => e.type === "meeting").length === 0 && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 justify-end mt-6">
              <button onClick={() => setShowEventModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={handleAddEvent} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">
                Add Event
              </button>
            </div>
          </div>
