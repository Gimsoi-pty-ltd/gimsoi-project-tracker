...
import { Plus, FileText, Clock, Filter, Download, Share2, MoreVertical } from "lucide-react";
  ...
  <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
    ...
    <div className="divide-y divide-gray-200">
      {documents.length > 0 ? (
        ...
        <div className="px-6 py-8 text-center text-gray-500">No documents yet</div>
      )}
    </div>
  </div>
);
...
const DocumentsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchDocs = async () => {
      setIsLoading(true);
      try {
        const response = await resourceAPI.get("/documents");
  ...
  const documentsByType = useMemo(() => {
    ...
    if (filterType !== "All") filtered = filtered.filter((d) => d.type === filterType.toLowerCase());
    if (sortBy === "updated") filtered = [...filtered].sort((a, b) => new Date(b.updatedDate) - new Date(a.updatedDate));
    else if (sortBy === "created") filtered = [...filtered].sort((a, b) => new Date(b.createdDate) - new Date(a.createdDate));
  ...
  const handleAddDocument = async () => {
    if (!newDoc.title || !newDoc.projectId) return;
    try {
      const response = await resourceAPI.post("/documents", newDoc);
      setDocuments((prev) => [...prev, response.data.document || response.data]);
    } catch {
      
      setDocuments((prev) => [...prev, { id: `local-${Date.now()}`, ...newDoc, status: "draft", createdBy: "You", updatedDate: new Date().toISOString().split("T")[0] }]);
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            {isLoading ? (
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-400">Loading documents...</div>
            ) : filterType === "All" ? (
          ...
          <div className="col-span-1 space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Filter size={18} />Document Types</h3>
                ...
                {["All", "Architecture", "API", "Guidelines", "Schema"].map((type) => (
                  <button
                    ...
                    className={`block w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition ${
                      filterType === type ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-100"
                    }`}
                  >
                    {type}
                  </button>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Clock size={18} />Recently Updated</h3>
            ...
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-sm p-6 text-white">
              ...
              <p className="text-sm text-blue-100 mb-4">Organize and share project documents with your team</p>
              <div className="space-y-2">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3 justify-end mt-6">
              <button onClick={() => setShowNewDocModal(false)} className="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
              <button onClick={handleAddDocument} className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700">Create Document</button>
            </div>
          </div>
        </div>
      )}
