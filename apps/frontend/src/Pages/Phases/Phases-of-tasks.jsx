    ...
    useEffect(() => {
        const fetchPhaseStatus = async () => {
            ...
            try {
                ...
                if (current) {
                    // The phases list endpoint doesn't include task counts — only the
                    // per-phase milestone endpoint does (this is what the Phases tab
                    // and Kanban board's phase completion sync both key off of).
                    // Without this, progress always fell back to 0%, showing a
                    // different status here than on the Phases tab.
                    try {
                        const milestoneRes = await resourceAPI.get(`/phases/${current.id}/milestone`);
                        const milestone = milestoneRes.data?.data ?? milestoneRes.data;
                        setActivePhase({ ...current, taskCounts: milestone?.taskCounts });
                    } catch (milestoneErr) {
                        console.error("Failed to fetch phase milestone status", milestoneErr);
