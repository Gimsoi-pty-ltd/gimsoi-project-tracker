...
export default function ProjectPhasesGantt() {
  const { projects, currentProject, fetchProjects, isLoading: projectsLoading } = useProjectStore();
  ...
  useEffect(() => {
    if (activeProject?.id) {
      getSprints({ projectId: activeProject.id });
    }
  }, [activeProject?.id]);

  
  const getProjectSprint = (projectId) =>
    sprints.find((s) => s.projectId === projectId && s.status === "active") ||
    sprints.find((s) => s.projectId === projectId) ||
    null;
      ...
      <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center mb-6 md:mb-8">
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-500 mt-1">
            ...
            {activeProject && (
              <> · Active project: <span className="font-medium text-blue-600">{activeProject.name}</span></>
            )}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-x-auto">
          <div className="grid grid-cols-6 md:grid-cols-7 min-w-full bg-gray-100 text-xs font-semibold text-gray-600 uppercase tracking-wider">
            <div className="col-span-2 p-3 md:p-4 border-r border-gray-200 text-left">Project & Lead</div>
          ...
          {phases.length === 0 ? (
            ...
            phases.map((phase) => (
              <div key={phase.id} className="grid grid-cols-6 md:grid-cols-7 min-w-full border-b border-gray-100 hover:bg-gray-50 transition items-center">
                <div className="col-span-2 p-3 md:p-4 border-r border-gray-200">
      ...
      {activeProject && (
        <div className="mt-4 md:mt-6 bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-5">
          <h2 className="font-semibold text-gray-800 mb-3 text-base md:text-lg">
            Active Sprint — {activeProject.name}
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 text-sm">
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-gray-500 text-xs uppercase tracking-wide mb-1">Phase</p>
              <p className="font-semibold text-gray-800 text-sm">{activeSprint?.currentPhase ?? "—"}</p>
            </div>
          </div>
        </div>
      )}
