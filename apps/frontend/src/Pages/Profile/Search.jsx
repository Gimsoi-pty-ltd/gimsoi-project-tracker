...
export default function SearchPage() {
  ...
  const filters = ["All", "project", "task", "client"];

  // Build searchable items dynamically from live store data
  const searchableItems = useMemo(() => [
    ...projects.map((p) => ({
      id: p.id,
      type: "project",
      title: p.name,
      description: `Status: ${p.status}${p.clientName ? ` · Client: ${p.clientName}` : ""}`,
      keywords: [p.name, p.status, p.clientName].filter(Boolean),
    })),
    ...tasks.map((t) => ({
      id: t.id,
      type: "task",
      title: t.title,
      description: `${t.status} · Priority: ${t.priority}`,
      keywords: [t.title, t.status, t.priority, t.assignee].filter(Boolean),
    })),
    ...clients.map((c) => ({
      id: c.id,
      type: "client",
      title: c.name,
      description: c.email ?? "",
      keywords: [c.name, c.email, c.location].filter(Boolean),
    })),
  ], [projects, tasks, clients]);

  // Filter results based on search query and type filter
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case "team-member": return <User className="w-5 h-5 text-green-500" />;
      case "document":    return <FileText className="w-5 h-5 text-orange-500" />;
      case "client":      return <User className="w-5 h-5 text-cyan-500" />;
              ...
              <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                <ul className="divide-y divide-gray-50">
                  {filteredResults.map((result, idx) => (
                    <li key={idx} className="p-4 md:p-5 hover:bg-blue-50/40 transition-colors cursor-pointer group">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-4">
                            <p className="text-base font-semibold text-gray-900 truncate">{result.title}</p>
                          </div>
          ...
          <div className="space-y-5 md:space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#2f66d0] to-indigo-600 rounded-2xl shadow-md p-6 text-white relative overflow-hidden">
              ...
              <p className="text-blue-100 text-sm mb-5 relative z-10 leading-relaxed">
                Search across projects, tasks, clients, and documents. Results update in real-time as you type.
              </p>
