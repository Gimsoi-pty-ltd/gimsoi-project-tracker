...
function TeamInsights() {
  ...
  const Card = ({ title, children }) => (
    <div className="flex h-full w-full flex-col bg-white rounded-2xl border border-gray-100 p-6 shadow-lg hover:shadow-xl transition-all duration-300">
  ...
  const StatBox = ({ icon, label, value, bg }) => (
    <div className={`${bg} rounded-xl p-4 flex h-full min-h-[104px] flex-col justify-between shadow-sm hover:scale-[1.02] transition-transform`}>
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 pb-6 md:pb-8">
        <div className="py-6 md:py-8">
          ...
          <p className="text-xs md:text-sm text-gray-500 mt-1">
            {currentProject?.name || "No Project"} - {activeSprint?.name || "No Active Sprint"} -{" "}
            {activeSprint?.goal || "No Goal Set"}
          </p>
        </div>

        <div className="grid w-full grid-cols-1 items-stretch gap-4 md:gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <Card title="Contribution Overview">
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
                ...
                <StatBox
                  label="Blocked"
                  value={activeSprint?.kanban?.blocked ?? 0}
                  bg="bg-orange-50"
                ...
                {memberStats.map((member) => (
                  <div key={member.id} className="flex text-sm py-3 px-4 border-b last:border-none hover:bg-gray-50 transition-colors">
                    <span className="flex-1 text-gray-700 flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-[#002D62] text-white flex items-center justify-center text-xs font-bold">
                        {member.initials}
                      </div>
                      {member.firstName}
                    </span>
            ...
            <Card title="Tasks Summary">
              <ul className="text-sm space-y-4 text-gray-600">
                {[
                  ...
                  {
                    label: "In Progress",
                    value: activeSprint?.kanban?.inProgress ?? "-",
          ...
          <div>
            <Card title="Notifications">
              <div className="flex flex-1 flex-col items-center justify-center text-gray-400 text-sm">
