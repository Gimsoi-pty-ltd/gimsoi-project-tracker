...
import { authAPI, resourceAPI, refreshCsrfToken } from "../api/api";
...
export const useAuthStore = create((set, get) => ({
  ...
  updateProfile: async (profileUpdates) => {
    ...
    try {
      ...
      if (currentVersion == null) {
        throw new Error(
          "Missing profile version — refresh your profile and try again.",
        );
      }
  ...
  changePassword: async (currentPassword, newPassword) => {
    set({ isLoading: true, error: null, message: null });
    try {
      const response = await resourceAPI.patch("/users/me/password", {
        currentPassword,
        newPassword,
      });
      set({
  ...
  addActivityLog: async (action, entityId = null, entityType = null) => {
    try {
      const response = await resourceAPI.post("/activity", {
        action,
        entityId,
        entityType,
      });
